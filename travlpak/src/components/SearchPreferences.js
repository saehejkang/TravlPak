...
class SearchPreferences extends Component {
  ...
  render() {
    /*var db = fire.firestore()
             ...
             fire.auth().onAuthStateChanged(function(user) {
               if (user) {
                  var intr = db.collection("Users").doc(user.uid).get(interests);
                  this.setState(interests: intr);
               }
             }
             });*/
    return (
      <div className="search-preferences">
        ...
        <input placeholder="San Luis Obispo, CA, US" className="my-location-box" type="location" value={this.location} onChange={(e) => this.setState({location: e.target.value }), this.firebase_update}/>
